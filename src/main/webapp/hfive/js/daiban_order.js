  ...
  $('#btn-add').on('click', function () {
    ...
    if (car_list) {
      ...
      if (car_list.list.length == 0) {
        node = '<div id="add-list"><div class="loading-box default"><p class="loading-text">正在获取可申请理赔的车辆</p><p class="default-text">暂无可申请理赔的车辆</p></div></div>';
      } else {
      ...
      showConfirm({
        ...
        btn_yes: {
          event_click: function event_click() {
            ...
            if (item.length > 0) {
              window.location.href = '/view/baoxian_order_add.html?id=' + item.eq(0).data('value');
            }
          }
        },
        btn_cancel: {
      ...
      showConfirm({
        ...
        btn_close: true,
        cover_background: 'rgba(255,255,255,0.5)',
        node: '<div id="add-list"><div class="loading-box loading"><p class="loading-text">正在获取可申请理赔的车辆</p><p class="default-text">暂无可申请理赔的车辆</p></div></div>',
        btn_yes: {
          event_click: function event_click() {
            var item = $('#add-list .item.active');
            if (item.length > 0) {
      ...
      getCarList(function (data) {
        ...
        if (car_list.total == 0) {
          ...
          car_list.list = $.map(car_list.list, function (item, index) {
            if (item.eventNo && item.statusEvent != 100) {
              return null;
            } else {
              return item;
            }
