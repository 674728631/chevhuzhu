...
(function () {
  ...
  function confirmData(dt) {
    ...
    goAPI({
      ...
      success: function success(data) {
        window.location.href = '/hfive/view/order_add2.html?id=' + cid + '&oid=' + (data.data ? data.data : oid);
      },
      error: function error(msg, code, data) {
        if (code == 488) {
          // 该车辆已经申请了理赔，引导用户跳转到对应的理赔订单页面
          showConfirm({
            str: plate + '的车辆已经有一个订单，请不要重复申请，谢谢。',
            btn_yes: {
              str: '查看订单',
              href: '/hfive/view/order_detail.html?id=' + data.data
            },
            btn_no: {
              str: '我知道了'
            }
          ...
          // 该车辆的理赔额度已用完，引导用户跳转到保险理赔申请页面
          showConfirm({
            str: msg,
            btn_yes: {
              str: '发起保险理赔',
              href: '/hfive/view/baoxian_order_add.html?id=' + cid
            },
            btn_no: {
              str: '我知道了'
            }
  ...
  function initData(data) {
    ...
    if (data.statusEvent && data.statusEvent != 2) {
      showConfirm({
        str: '只有未通过审核的救助申请才可以进行修改',
        btn_yes: {
          str: '返回上一页',
          href: 'javascript:history.go(-1);'
        },
    ...
    $('#btn-confirm').on('click', function () {
      ...
      if (img3.length === 0 || img4.length === 0) {
        showAlert('请上传两张受损处细节照片');
        return;
      }
      $([img1, img2, img3, img4]).each(function (index, item) {
        var img = $(item);
        var src = img.attr('src');
        if (/^http/.test(src)) {
          base.push(src.match(/.+\/(.+\.jpe?g|.+\.png)/)[1]);
