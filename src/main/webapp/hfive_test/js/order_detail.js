...
(function () {
  ...
  function setOrderStep(status) {
    ...
    if (status == 1 || status == 2) {
      step = 1;
    } else if (status == 3 || status == 10 || status == 11 || status == 12 || status == 21 || status == 22 || status == 23) {
      step = 2;
    } else if (status == 31) {
      step = 3;
    } else if (status == 41 || status == 51 || status == 52) {
      step = 4;
    } else {
      step = 5;
    }
      ...
      success: function success(data) {
        showConfirm({
          ...
          btn_no: {
            str: '去评价',
            event_click: function event_click() {
              window.location.href = '/hfive/view/score.html?id=' + oid;
            }
          }
        });
      },
  ...
  function loadData() {
    ...
    goAPI({
      ...
      success: function success(data) {
        ...
        if (data_order.statusEvent == 4 && data_order.isInvalid != 10) {
          window.location.href = '/hfive/view/order_add2.html?oid=' + oid;
          return;
        }
        ...
        if (data_order.isInvalid != 10) {
          setOrderStep(data_order.statusEvent);
        }
        if ($('#time').length > 0) {
