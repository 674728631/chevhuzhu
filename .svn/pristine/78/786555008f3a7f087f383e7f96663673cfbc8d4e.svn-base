    ...
    <div id="page-wrapper">
        ...
        <div class="row margin-row-max">
            <div class="col-sm-4 new-search-btn">
                ...
                <button type="button" class="btn def-btn btn-success" onclick="listData(1)">搜索</button>
            </div>
            <div class="col-sm-4">
            ...
            <div class="col-lg-12 col-xs-12">
                <div>
                    ...
                    <ul class="nav nav-tabs nav-bac" role="tablist">
                        ...
                        <li role="presentation" id="flag1"><a id="applyNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(1,'1','1')">申请代办</a></li>
                        ...
                        <li role="presentation" id="flag7"><a id="deliverCarNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(7,'1','21')">待接车</a></li>
                        <li role="presentation" id="flag8"><a id="waitAssertNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(8,'1','31')">待定损</a></li>
                        <li role="presentation" id="flag9"><a id="assertNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(9,'1','32')">待确认</a></li>
                        <li role="presentation" id="flag10"><a id="waitRepairNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(10,'1','41')">待维修</a></li>
                        <li role="presentation" id="flag11"><a id="repairNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(11,'1','42')">维修中</a></li>
                        <li role="presentation" id="flag12"><a id="payNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(12,'1','51')">待交车</a></li>
                        <li role="presentation" id="flag13"><a id="commentNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(13,'1','61')">待评价</a></li>
                        <li role="presentation" id="flag14"><a id="complaintNum" href="#home_1" aria-controls="home_1" role="tab" data-toggle="tab" onclick="changeTab(14,'1','71')">投诉中</a></li>
...
<form class="form-horizontal margin-top-ele lay-form-time" action="${ctx}/excel/order" method="post" id="excelExport" style="display: none"></form>
<script type="text/javascript">
    ...
    function datacount(searchInfo) {
        ...
        success : function(datas){
            if(datas.code == "0"){
                ...
                $("#applyNum").html("申请代办("+result.applyNum+")");
    ...
    function listData(pageNo){
        ...
        datacount(searchInfo);
        var strJson = {"pageNo":pageNo,"pageSize":pageSize,"searchInfo":searchInfo,"isInvalid":isInvalid,"status":status};
        $.ajax({
            type : "post",
            dataType : "json",
            headers: {rqSide: ex.pc()},
            ...
            success : function(datas){
                if(datas.code == "0"){
                    ...
                    var dataHtml = "<thead><tr><th>车主姓名</th><th>手机号</th><th>车牌号</th><th>车型品牌</th><th>渠道来源</th><th>维修厂</th><th>状态</th><th>时间</th><th>操作</th></tr></thead>";
                    dataHtml += "<tbody>";
                    if (0 == repData.length) {
                        dataHtml += "</tbody>";
                        ...
                        for (var i = 0; i < repData.length; i++) {
                            ...
                            var orderStatus = ex.orderStatus(repDateChild.status);
                            var sourceName = ex.judgeEmpty(repDateChild.sourceName) ? repDateChild.sourceName : "自然用户";
                            var day = ex.judgeEmpty(repDateChild.applyTime) ? ex.calcDays(repDateChild.applyTime.time) : 0;
                            dataHtml += "<tr>" +
                                ...
                                "<td>" + orderStatus + "</td>" +
                                "<td>" + day + "天</td>" +
                                "<td><button type='button' class='btn btn-info'onclick=showDetail(" + repDateChild.status + ",'" + ex.judgeEmpty(repDateChild.orderNo) + "')>查看</button></td>" +
                                "</tr>";
                        }
                        dataHtml += "</tbody>";
                        $("#mainDataTable").html(dataHtml);
    ...
    function changeTab(flag,isInvalid,status) {
        var searchInfo = $("#searchInfo").val();
        self.location.href = "${ctx}/order/list.html?flag=" + flag + "&isInvalid=" + isInvalid + "&status=" + status + "&searchInfo=" + searchInfo;
    }

    //查看详情
    function showDetail(status,orderNo) {
        self.location.href = "${ctx}/order/detail.html?status=" + status + "&orderNo=" + orderNo;
    }
